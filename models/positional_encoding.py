...
class PositionalEncodingsFixed(nn.Module):
    ...
    def _1d_pos_enc(self, mask, dim):
        ...
        temp = self.temperature ** (
            2 * (temp.div(2, rounding_mode="floor")) / self.emb_dim
        )

        enc = (~mask).cumsum(dim).float().unsqueeze(-1) / temp
        enc = torch.stack([enc[..., 0::2].sin(), enc[..., 1::2].cos()], dim=-1).flatten(
            -2
        )
    ...
    def forward(self, bs, h, w, device):
        mask = torch.zeros(
            bs, h, w, dtype=torch.bool, requires_grad=False, device=device
        )
