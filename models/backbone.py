...
class Backbone(nn.Module):
    ...
    def __init__(self, name: str, pretrained: bool, dilation: bool, reduction: int, swav: bool, requires_grad: bool):

        super(Backbone, self).__init__()

        resnet = getattr(models, name)( replace_stride_with_dilation=[False, False, dilation],
        ...
        self.num_channels = 896 if name in ['resnet18', 'resnet34'] else 3584 # resnet 50 use 3584
    ...
    def forward(self, x):
        ...
        x = torch.cat([F.interpolate(f, size=size, mode='bilinear', align_corners=True) for f in [layer2, layer3, layer4]], dim=1)
